...
describe('Menu tests', function() {
    ...
    describe('ability to navigate through menu items with tab and arrow keys', function() {
        it('should navigate through menu items when user clicks arrow down and arrow up', () => {
            for (let i = 0; i < buttonsArrLength; i++) {
                click(menuButtonsArr, i);
                const menuItemsArrLength = getElementArrayLength(menuItemsArr);
                for (let j = 0; j < menuItemsArrLength; j++) {
        ...
        it('should navigate through menu items when user clicks tab', () => {
            for (let i = 0; i < buttonsArrLength; i++) {
                click(menuButtonsArr, i);
                const menuItemsArrLength = getElementArrayLength(menuItemsArr);
                for (let j = 0; j < menuItemsArrLength; j++) {
    ...
    function checkItemsWithKey(items: string, key: string, style: string, valueCss: string, index: number) {
        setValue(items, key, index);
        const styleValue = getCSSPropertyByName(items, style, index).value;
        expect(styleValue).toBe(valueCss);    
    }
});
